...
Check command polling frequency in config:
```yaml
command_check_every_n_steps: 50  # Lower = more frequent checks
```

## Future Enhancements

1. **Real-time LLM Integration:** Stream commands during simulation instead of pre-planned
2. **Sensor Fusion:** Feed drone sensor data to LLM for dynamic decision making
3. **Multi-modal Communication:** Voice commands → LLM → swarm control
4. **Adversarial Scenarios:** Dynamic threats requiring real-time LLM reasoning
5. **Formation Optimization:** LLM suggests optimal formations based on terrain/mission
6. **Energy Awareness:** LLM considers battery levels in command decisions
...
```
@misc{asen6519_fire_patrol,
  title={LLM-Driven Multi-Group Drone Swarm Control for Fire Patrol},
  author={Your Name},
  year={2025},
  note={ASEN 6519 Swarming Project}
}
```
